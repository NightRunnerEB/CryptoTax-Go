...
func (r *importStateRepo) Get(ctx context.Context, userID, importID uuid.UUID) (domain.AggregationImportState, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return domain.AggregationImportState{}, apperr.NotFound("import state not found", apperr.Resource{
				...
				Name: userID.String() + ":" + importID.String(),
			}, err)
		}
...
func (r *importStateRepo) MarkCompleted(ctx context.Context, userID, importID uuid.UUID) error {
...
func (r *importStateRepo) MarkFailed(ctx context.Context, userID, importID uuid.UUID, errMsg string) error {
	msg := errMsg
	if msg == "" {
		msg = "unknown error"
	}
