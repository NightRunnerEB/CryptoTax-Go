...
func setupIntegrationStore(t *testing.T) db.Store {
	...
	dsn := strings.TrimSpace(os.Getenv("PRICE_SVC_TEST_DATABASE_URL"))
	if dsn == "" {
		t.Skip("PRICE_SVC_TEST_DATABASE_URL is not set; skipping integration test")
	}
	...
	schema := "price_it_" + strings.ReplaceAll(uuid.NewString(), "-", "")
