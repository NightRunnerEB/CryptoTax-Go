...
func (d *OutboxDispatcher) routingKeyFor(eventType string) string {
	switch eventType {
	case domain.EventTypeTaxReportJobRequested:
		return d.cfg.RoutingJobRequested
	case domain.EventTypeReportRenderRequested:
		return d.cfg.RoutingRenderRequest
...
func backoffForAttempts(attempts int32) time.Duration {
	...
	// Exponential backoff with cap: 1s,2s,4s,8s... max 1m.
	d := time.Second
